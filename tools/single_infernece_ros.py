...
def remove_low_score_nu(image_anno, thresh):
    ...
    pedestrain_indices =           get_annotations_indices(8, 0.4, label_preds_, scores_)
...
def rslidar_callback(msg):
    ...
    # result = proc_2.step_centertrack(result, 0.1)
    result = proc_mot.track_one_frame(result)

    result[:,8] = -result[:,8] - np.pi / 2
    proc_Drawbox.display(result, 0.1)
    result[:, 8] = -result[:, 8] - np.pi / 2
    if scores.size != 0:
    ...
    #         bbox.header.frame_id = msg.header.frame_id
    #         bbox.header.stamp = rospy.Time.now()
    #         q = yaw2quaternion(float(result[i][8]))
    #         bbox.pose.orientation.x = q[1]
    #         bbox.pose.orientation.y = q[2]
    #         bbox.pose.orientation.z = q[3]
    #         bbox.pose.orientation.w = q[0]
    #         bbox.pose.position.x = float(result[i][0])
    #         bbox.pose.position.y = float(result[i][1])
    #         bbox.pose.position.z = float(result[i][2])
    #         bbox.dimensions.x = float(result[i][4])
    #         bbox.dimensions.y = float(result[i][3])
    #         bbox.dimensions.z = float(result[i][5])
    #         bbox.value = result[i][11]
    #         bbox.label = int(result[i][9])
    #         arr_bbox.boxes.append(bbox)
...
if __name__ == "__main__":
    ...
    proc_1.initialize()
    proc_2 = cpTracker.PubTracker(max_age=3, hungarian=False)
    proc_mot = mot.Mot3D('tools/tracking/pvrcnn_mot.yaml')
    proc_Drawbox = ab.DrawBoundingBox()
    ...
    pub_pointcloud = rospy.Publisher("pp_points", PointCloud2, queue_size=1)
